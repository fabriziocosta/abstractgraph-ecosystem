...
| P1-C6 | Selected graph-valued compositions add a localized discrimination or Pareto-frontier point beyond their atomic components and lossless feature concatenation. | For every composite, compare atoms, lossless concatenation, and pre-vectorization graph composition; require held-out separation or localization unavailable to both controls. If none exists, revise the paper to claim organizational unification only. | `fig03-composition-transitions.pdf`; `fig04-pareto-frontiers.pdf`; E01/E02 raw results. | Abstract; Introduction; RQ1--RQ2; Results; Discussion. | Proposed |
| P1-C7 | Selected path, WL, and NSPDK representations have faithful Abstract Graph translations under explicitly matched semantics. | Independent parity checks of multiplicities, graph-pair decisions, and kernel/vector values; methods failing parity are excluded from comparative claims. | `tab01-baseline-expressions.tex`; E01 parity results. | Introduction; Sections 4--6. | Proposed |
...
| P1-C6 | P1-C1, P1-C2, P1-C5 | A compositional gain must be mapped, well-formed, and compared at matched cost. |
| P1-C7 | P1-C2, P1-C3 | Baseline translations require valid programs and matched identity semantics. |
