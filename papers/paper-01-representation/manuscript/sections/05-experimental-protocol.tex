...
A random forest is a secondary nonlinear probe. If signatures differ but only
the forest succeeds, information may require nonlinear interactions. If both
the signatures and probes agree, the distinction between representational
collapse and model failure is explicit. Feature vocabularies are fitted on
training data only.
...
density, and label histogram. Candidate real-world datasets complement these
controlled tests but do not replace them.
...
graph-pair decisions, and kernel or vector values where applicable. Composite
expressions then test, for example, cycle features combined with WL or path
features combined with pairs of neighborhoods.
...
boundaries. We report separate discrimination--runtime,
discrimination--memory, and discrimination--size Pareto frontiers. Raw costs
